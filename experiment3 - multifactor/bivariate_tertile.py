...
``composite.py`` route) but by an *independent double sort*: every month the
industry cross-section is split into three equal-count tertiles on the first
factor and, **separately**, into three tertiles on the second.  This overlays a
3x3 grid on the cross-section.  The book is
...
----------------------------------------------
    exposure_correlation.csv    correlation of the two oriented constituent z-scores
    grid_mean_return.png        3x3 heatmap: mean next-month return of each tertile cell,
                                ...
                                and number of names
    bivariate_returns.csv       months x {long, short, long_short, n_long, n_short}
    long_short.png              the long-short book's cumulative growth of $1
    performance.png             the book's mean / t / Sharpe / industry-neutral alpha /
                                alpha above the gross_profitability & revenue_stability
                                books (with t-stats) / avg cost
    half/                       the half-intersection (median double-sort) rule's
        ...
        half/returns.csv            months x {long, short, long_short, ...}
        half/long_short.png         the book's cumulative growth of $1
...
# --------------------------------------------------------------------------- #
def double_sorted(resolved: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Tidy ``date, stock_id, <fa>, <fb>, next_return, mcap, tile_a, tile_b`` frame
    plus the wide oriented-exposure matrix.
    ...
    frame = frame.dropna(subset=["tile_a", "tile_b"])
    return frame, oriented
...
def evaluate_book(spread: pd.Series, cost_series: pd.Series, industry: pd.Series,
                  benchmarks: dict[str, pd.Series]) -> tuple[list, dict, dict]:
    """
    ...
    turnover cost over that window and the alpha the book earns *above* each
    standalone benchmark book (:func:`benchmark_alphas`).  Shared by both the
    tertile-corner and the half-intersection selection rules so they are measured
    identically.  Returns ``(windows, full_stats, decade_stats)``.
    """
...
def plot_long_short(spread: pd.Series, factor_names: list[str], sharpe: float,
                    ...
                    rule_label: str = "Bivariate tertile long-short "
                    "(long top-tertile-of-both / short bottom-tertile-of-both)") -> None:
    """Cumulative growth of $1 in the dollar-neutral double-sort long-short book."""
    cum = (1.0 + spread.fillna(0.0)).cumprod()
    fig, ax = plt.subplots(figsize=(11, 5))
    ax.plot(cum.index, cum, color="C3", linewidth=1.3)
    ax.axhline(1.0, color="black", linewidth=0.6)
    ax.set_title(f"{rule_label}: "
...
def run(factor_names: list[str] = DEFAULT_FACTORS,
    ...
    oriented.corr().to_csv(out_dir / "exposure_correlation.csv")
    plot_grid(grids, factor_names, out_dir / "grid_mean_return.png")
    book.to_csv(out_dir / "bivariate_returns.csv")
    ...
    hbook.to_csv(half_dir / "returns.csv")
    plot_long_short(hspread, factor_names, hfull["sharpe"], hfull["alpha"],
                    hfull["alpha_tstat"], half_dir / "long_short.png",
                    rule_label="Bivariate half-intersection long-short "
                    "(long top-half-of-both / short bottom-half-of-both)")
    C.render_performance(
        hwindows,
