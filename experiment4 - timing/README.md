...
|---|---|---|---|
| `spread_timing.py` | the factor's own **value spread** — the monthly Q5−Q1 gap in raw (un-z-scored) factor values | the spread is **above its trailing 6-month average** (wide dispersion = more to be paid for ranking on the factor) | **every** ranked factor (`factor_ranking/monthly_quintile_ranked.csv`) |
| `vol_timing.py` | the market's **volatility regime** — the VVIX index, 10-trading-day moving average (from `data/VolatilityIndexData.csv`) | the VVIX MA is **above 95** (elevated vol-of-vol regime) | the **top 5** factors (`factor_ranking/monthly_quintile_ranked.csv`) |
...
`factor_ranking/monthly_quintile_ranked.csv`, and (for vol timing)
